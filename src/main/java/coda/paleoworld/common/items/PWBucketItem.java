...
public class PWBucketItem extends BucketItem {
    private final boolean hasTooltip;
    private final Supplier<? extends Fluid> fluid;

    public PWBucketItem(Supplier<? extends EntityType<?>> entityType, Supplier<? extends Fluid> fluid, Properties builder) {
        this(entityType, fluid, builder, true);
    }

    public PWBucketItem(Supplier<? extends EntityType<?>> entityType, Supplier<? extends Fluid> fluid, Properties builder, boolean hasTooltip) {
        super(fluid, builder);
        this.fluid = fluid;
        this.hasTooltip = hasTooltip;
        this.entityTypeSupplier = entityType;
        DistExecutor.unsafeCallWhenOn(Dist.CLIENT, () -> () -> Paleoworld.CALLBACKS.add(() -> ItemModelsProperties.register(this, new ResourceLocation(Paleoworld.MOD_ID, "variant"), (stack, world, player) -> stack.hasTag() ? stack.getTag().getInt("Variant") : 0)));
    }
